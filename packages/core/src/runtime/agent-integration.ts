...
function renderAgentsInstructions(config: ContextProjectConfig): string {
  ...
  return [
    ...
    '- Use package-first MCP tools first: `list_context_packages`, `get_context_package`, `expand_context_package`, and `search_context_package`.',
    '- After package drill-down, use package correction tools (`list_package_corrections`, `get_correction_proposal`, `preview_correction_proposal`, `approve_correction_proposal`, `reject_correction_proposal`, `apply_correction_proposal`) before graph patch tools.',
...
function renderClaudeInstructions(): string {
  return [
    ...
    '- Start package-first with `list_context_packages`, then use `get_context_package`, `expand_context_package`, or `search_context_package`.',
    '- Review correction proposals with `list_package_corrections`, `get_correction_proposal`, and `preview_correction_proposal`; approve, reject, or apply through package correction tools before graph patch tools.',
...
function renderCodexExplorerAgent(): string {
  return [
    ...
    'Then use list_context_packages, get_context_package, expand_context_package, or search_context_package before graph scope tools.',
    'After package drill-down, use list_package_corrections, get_correction_proposal, and preview_correction_proposal before low-level graph patch tools.',
...
function renderSkill(skill: ContextSkillDefinition): string {
  ...
  return [
    ...
    '- Major work should align with `docs/architecture/super-data-network-goal.md`.',
    '- Start with `get_context_health` and `get_context_manifest`.',
    '- Use package-first tools (`list_context_packages`, `get_context_package`, `expand_context_package`, `search_context_package`) before graph scope tools.',
    '- Use package correction tools (`list_package_corrections`, `get_correction_proposal`, `preview_correction_proposal`, `approve_correction_proposal`, `reject_correction_proposal`, `apply_correction_proposal`) before low-level graph patch tools.',
    '- Use `search_context`, `get_task_context`, and `get_source_trace` for focused evidence once the relevant package is known.',
